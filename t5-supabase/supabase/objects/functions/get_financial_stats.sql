...
CREATE OR REPLACE FUNCTION get_financial_stats(input_date text, timezone text)
RETURNS TABLE (
  id bigint,
  name text,
...
with ledger_summary as (
  select
    l.id,
    l.name,
    sum(case when t.type = 'debit' then -t.amount else t.amount end) as current_balance,
    sum(case when t.type = 'debit' and date_trunc('month', t.created_at at time zone '+07') = date_trunc('month', now() at time zone '+07') then t.amount else 0 end) as current_month_debit,
    sum(case when t.type = 'credit' and date_trunc('month', t.created_at at time zone '+07') = date_trunc('month', now() at time zone '+07') then t.amount else 0 end) as current_month_credit
  from ledgers l
