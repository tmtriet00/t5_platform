        ...
        html5QrCode.start({ facingMode: "environment" }, config, (decodedText) => {
            ...
            .catch(err => {
                ...
                if (err?.name === "NotAllowedError" || err?.toString().includes("Permission")) {
                    errorMsg = "🚫 Permission Denied. Please reset browser permissions.";
                    if (!isSecure) {
                        errorMsg += "<br><br><b>⚠️ IMPORTANT:</b> Mobile browsers BLOCK camera access on insecure (HTTP) sites. You MUST use HTTPS.";
                    }
                } else if (err?.name === "NotFoundError") {
                    errorMsg = "📷 No camera found.";
                } else if (err?.name === "NotReadableError") {
                    errorMsg = "⚠️ Camera is in use by another app or hardware error.";
                }
